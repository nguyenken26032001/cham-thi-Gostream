...
const delQuest = (item) => {
    round.questions.value = round.questions.value.filter((val) => val.id !== item.id);
};
...
const saveRound = () => {
    ...
    if (round.value.questions.length == 0)
        return;
    if (round.value._id) {
        HTTP.put(`rounds/${round.value._id}`, round.value)
            .then(res => {
                rounds.value.find(el => {
                    if (el._id = res.data.round._id)
                        el = res.data.round
                });
                round.value = {};
    ...
    HTTP.post('rounds/create', round.value)
        .then(res => {
...
<template>
    ...
    <div class="btn-add">
        <Button label="Thêm vòng thi" icon="pi pi-plus" class="p-button-success" @click="roundsDialog = true;" />
    </div>
