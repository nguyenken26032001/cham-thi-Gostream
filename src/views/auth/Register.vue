    ...
    methods: {
        register() {
            ...
            if (this.data.userName !== '' || this.data.email !== '' || this.data.password !== '') {
                axios
                    .post('http://localhost:3000/api/users/register', this.data)
                    .then((res) => {
        ...
        <div class="border-1 surface-border surface-card border-round py-7 px-4 md:px-7 z-1">
            ...
            <div class="flex flex-column">
                <span class="p-input-icon-left w-full mb-4">
                    ...
                    <InputText id="username" type="text" class="w-full md:w-25rem" placeholder="Username" v-model="data.userName" />
                </span>
                <span class="p-input-icon-left w-full mb-4">
                ...
                <span class="p-input-icon-left w-full mb-4">
                    ...
                    <Password id="password" type="password" class="w-full md:w-25rem input-password" placeholder="Password" v-model="data.password" toggleMask :feedback="false" />
                </span>
                ...
                <span class="font-medium text-600"> Already have an account? <a class="font-semibold cursor-pointer text-900 hover:text-primary transition-colors transition-duration-300" @click="toLogin">Login</a> </span>
            </div>
        </div>
    </div>
</template>
